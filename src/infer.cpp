...
YOLO::YOLO(const std::string& engine_file_path)
{
    ...
    this->mEngine.reset(this->runtime->deserializeCudaEngine(engineData.data(), fsize, nullptr));
    assert(this->mEngine.get() != nullptr);
    this->context.reset(this->mEngine->createExecutionContext());

    assert(this->context != nullptr);
    modelState = true;
    cudaStreamCreate(&this->stream);
    this->num_bindings = this->mEngine->getNbBindings();
...
void YOLO::make_pipe(bool warmup)
{
    for(auto& bindings : this->input_bindings)
    {
        ...
        CHECK(cudaMalloc(&input_mem, bindings.size * bindings.dsize));
        this->device_ptrs.push_back(input_mem);
    }

    for(auto& bindings : this->output_bindings)
    {
        void* output_mem{nullptr}, *output_mem_host{nullptr};
        CHECK(cudaMalloc(&output_mem, bindings.size * bindings.dsize));
        CHECK(cudaHostAlloc(&output_mem_host, bindings.size * bindings.dsize, 0));
        this->device_ptrs.push_back(output_mem);
        this->host_ptrs.push_back(output_mem_host);
    }
...
void YOLO::infer(void)
{
    this->context->enqueueV2(this->device_ptrs.data(), this->stream, nullptr);
    for(int i = 0; i < this->num_outputs; i++)
    {
        size_t osize = this->output_bindings[i].size * this->output_bindings[i].dsize;
        CHECK(cudaMemcpyAsync(this->host_ptrs[i], device_ptrs[i + this->num_inputs], osize, cudaMemcpyDeviceToHost, this->stream));
    }
...
void YOLO::letterbox(const cv::Mat& image, cv::Mat& out, cv::Size& size)
{
    ...
    float r    = std::min(inp_h / height, inp_w / width);   // Keep the longest side acting as effective side
...
void YOLO::postprocess(std::vector<Object>& objs, float score_thres, float iou_thres, int topk, int seg_channels, int seg_h, int seg_w)
{
    objs.clear();
    ...
    bool flag = false;
    int  bid;   // output binding id
    int  bcnt = -1;     // binding count
    ...
    int     cnt = 0;
    for (auto& i : indices) {
        if (cnt >= topk) {      // topk aims to limit the number of objects
            break;
        }
    ...
    if (masks.empty()) {
        ...
        cv::Mat matmulRes = (masks * protos).t();   // python: nx32 @ 32x(160x160) -> nx(160x160) -> sigmoid -> nx160x160
